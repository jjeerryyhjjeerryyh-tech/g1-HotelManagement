...
function createRoomCard(room) {
    ...
    const rating = room.rating || { average: 0, count: 0 };
        ...
        <div class="room-card">
            ...
            <div class="room-content">
                ...
                <div class="room-rating">
                    ...
                    <span class="rating-count">(${rating.count}条评价)</span>
                </div>
...
function viewRoomDetail(roomId) {
    ...
    // 生成评论列表HTML
    let reviewsHtml = '';
    if (currentRoom.reviews && currentRoom.reviews.length > 0) {
        reviewsHtml = currentRoom.reviews.map(r => `
            <div class="review-item">
                ...
                <div class="review-content">${r.comment || '无评论'}</div>
            </div>
        `).join('');
    } else {
        reviewsHtml = '<div class="review-empty">暂无评论，快来发表第一条评价吧！</div>';
    }
    ...
    body.innerHTML = `
        ...
        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1rem;">
            <div>
                ...
                <div class="room-rating" style="margin-top: 0.25rem;">
                    <span class="stars">${renderStars(currentRoom.rating.average)}</span>
                    <span class="rating-count">(${currentRoom.rating.count}条评价)</span>
                </div>
        ...
        <h4 style="margin-top: 2rem;">用户评价</h4>
...
function highlightStars(value) {
    ...
    stars.forEach((star, idx) => {
        if (idx < value) {
            star.className = 'fas fa-star';   // 点亮
        } else {
            star.className = 'far fa-star';   // 灰色空心
        }
    });
}
...
function openRatingModal(booking) {
    ...
    selectedRating = 0;
    document.getElementById('ratingRoomName').innerText = `房间：${booking.roomName}`;
...
function submitRating() {
    ...
    if (!currentBooking) {
        showToast('预订信息丢失', 'error');
        return;
    }

    const room = roomsData.find(r => r.id === currentBooking.roomId);
    if (!room) return;

    const comment = document.getElementById('ratingComment').value.trim();
    const author = currentBooking.guestName || '匿名用户';
    ...
    const newAvg = total / room.reviews.length;
    room.rating.average = Math.round(newAvg * 10) / 10;
    room.rating.count = room.reviews.length;

    showToast(`评分成功！您给了 ${selectedRating} 星`, 'success');
    
    renderRooms();
    if (document.getElementById('roomModal').classList.contains('active') && currentRoom && currentRoom.id === room.id) {
        viewRoomDetail(room.id); // 刷新详情页评论
    }
...
function renderMyBookings() {
    ...
    tbody.innerHTML = myBookings.map(b => {
        const ratingBtn = b.status === 'completed' 
            ? `<button class="btn btn-success btn-sm" onclick='openRatingModal(${JSON.stringify(b).replace(/'/g, "&#39;")})'>
        ...
        <tr>
            ...
            <td>
                <div style="display: flex; gap: 0.5rem;">
                    ${ratingBtn}
                    <button class="btn btn-danger btn-sm" onclick="showToast('取消功能暂未实现', 'info')">取消</button>
                </div>
            </td>
        </tr>
    `}).join('');
...
function filterMyBookings() {
    ...
    if (status === 'all') {
        ...
        tbody.innerHTML = filtered.map(b => {
            const ratingBtn = b.status === 'completed' 
                ? `<button class="btn btn-success btn-sm" onclick='openRatingModal(${JSON.stringify(b).replace(/'/g, "&#39;")})'><i class="fas fa-star"></i> 评价</button>`
                : '';
            return `<tr>...</tr>`; // 简化，实际可复用上面逻辑
        }).join('');
    }
}

// ============================================
// Modal Helpers (Lookup, Cancellation, Toast)
// ============================================
